...
### A3 `[로컬+DB]` / `[배포]` — 결과 PDF 저장

1. COMPLETED 진단의 `/diagnosis/<id>` 결과 화면에서 "결과 PDF로 저장" 버튼 클릭.
2. 다운로드된 PDF 를 열어 내용 확인 (GA4 는 쿠키 동의 후 DebugView/Network 에서 확인).

- 기대: 클릭 즉시 `webagent-진단결과-<ID 앞 8자>.pdf` 다운로드, 한글 정상 표시, 이름·전화·이메일 없음, "추정치" 고지 문구 포함, GA4 `result_pdf_download` 발화(동의 시).
- **결과:**
