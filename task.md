...
- [~] 4.5 Contabo Docker 실제 배포 + **DB 백업 (D5: Free + cron `pg_dump` 일 1회, 7일 로테이션, 오프사이트 복사)** — P0 항목이므로 출시 전 완료 필요(결함 #14, #18)
  ...
  - **미완료**: 오프사이트 복사(현재 VPS 로컬 디스크만 — 사용자 결정으로 다음 단계로 이연). Portainer + `n8n.webagent.kr`/`portainer.webagent.kr` 공개 서브도메인화는 설치까지 마쳤으나 SSL 발급이 도메인 DNS 네거티브 캐시(SOA minimum 86400s) 때문에 보류 — 지금은 SSH 터널로만 n8n/Portainer/NPM 관리 접근, 캐시 만료 후 재시도 필요. 첫 계약 성사 시 Supabase Pro 전환은 미착수.
